...
static inline u8 has_new_bits(u8* virgin_map) {
  ...
  u64* current = (u64*)trace_bits; // trace_bits 是 65536 个 uchar，用 64 位指针来操作这个数组的话，意味着每次操作64位，即8个数组元素
  ...
  while (i--) { // 64位 需要迭代 65536/8 = 8192 次 

    /* Optimize for (*current & *virgin) == 0 - i.e., no bits in current bitmap
       that have not been already cleared from the virgin map - since this will
       almost always be the case. */

    if (unlikely(*current) && unlikely(*current & *virgin)) { //如果有新东西
  ...
  if (ret && virgin_map == virgin_bits) bitmap_changed = 1; //【坑】第一个条件ret不为0可以理解，第二个条件就有点无法理解
...
static void update_bitmap_score(struct queue_entry* q) {
  ...
  // maintain top_rated for every byte in trace_bits

  for (i = 0; i < MAP_SIZE; i++)
    // assumption is trace_bits must represent execution of 'q'
    if (trace_bits[i]) {
...
static void cull_queue(void) {
  ...
  static u8 temp_v[MAP_SIZE >> 3]; // 65536/8
  u32 i;

  if (dumb_mode || !score_changed) return;
  //perform queue culling only if score is changed

  score_changed = 0;

  memset(temp_v, 255, MAP_SIZE >> 3);
  //initialize all bits of temp_v to 1, 全 1 表示还没有被覆盖到，为 0 就表示被覆盖到了【这里的覆盖指的是分支的覆盖？对！】
  ...
  // i 从 0 到 MAP_SIZE 进行迭代，筛选出一组队列条目，它们可以覆盖所有现在
  // 已经覆盖的路径
  for (i = 0; i < MAP_SIZE; i++)
    if (top_rated[i] && (temp_v[i >> 3] & (1 << (i & 7)))) {
      ...
      if (!top_rated[i]->was_fuzzed) pending_favored++; //被执行过了，但是还没有被变异过的case

    }

  q = queue;

  while (q) { // 遍历队列，不是favored的case（冗余的case）会被标记为redundant_edges
...
EXP_ST void setup_shm(void) {
  ...
  if (!in_bitmap) memset(virgin_bits, 255, MAP_SIZE); //初始化 virgin_bits 所有位都是1
...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  u8* old_sn = stage_name; //当前进行的阶段的名称
  //save old globals
  ...
  //update timeout value for some cases, not really important

  q->cal_failed++;

  stage_name = "calibration";
  stage_max  = fast_cal ? 3 : CAL_CYCLES; // the latter is 8
  ...
  //last execution must use case that 'q' represents.
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    if (q->exec_cksum != cksum) {
        // if the new cksum is different from cksum in 'q'
...
static u32 calculate_score(struct queue_entry* q) {
  ...
  switch (q->depth) { // 怎么理解 inpiut depth？
...
static u8 fuzz_one(char** argv) { // 对一个case的fuzz
  ...
  if (pending_favored) {  // 如果有还没有被 fuzz(变异) 过，但已经执行过且被标记为 favored 的 case，这些是重点 fuzz(进行变异) 的对象

    /* If we have any favored, non-fuzzed new arrivals in the queue,
       possibly skip to them at the expense of already-fuzzed or non-favored
       cases. */
      // 注意是 skip to them，也就是说要跳过 already-fuzzed or non-favored cases，然后变异那些 favored, non-fuzzed cases

    if ((queue_cur->was_fuzzed || !queue_cur->favored) &&
        UR(100) < SKIP_TO_NEW_PROB) return 1; // UR(100), 0-100 之前的随机数， 只要小于 99，就跳过 

  } else if (!dumb_mode && !queue_cur->favored && queued_paths > 10) { // 等待 fuzz 的 cases 有 10 个以上
...
#define FLIP_BIT(_ar, _b) do { \
    u8* _arf = (u8*)(_ar); \
    u32 _bf = (_b); \
    _arf[(_bf) >> 3] ^= (128 >> ((_bf) & 7)); \ //是从高位向低位依次进行翻转，从左往右
  } while (0)

  // FLIP_BIT 的作用就是翻转指定位置的 1 bit
  /* Single walking bit. */

  stage_short = "flip1";
  stage_max   = len << 3; // 文件长度(len)*8，推断 stage_max 是可以变异的次数，初步推测，原来的 len 的单位是 byte，然后 stage_max 的单位是 bit
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    stage_cur_byte = stage_cur >> 3;  //遍历到当前第几个字节了？

    FLIP_BIT(out_buf, stage_cur); // 翻转当前字节的第 stage_cur 个 bit
    ...
    if (!dumb_mode && (stage_cur & 7) == 7) {
      // (stage_cur & 7) == 7 意思就是，翻转到当前字节的最低位时
